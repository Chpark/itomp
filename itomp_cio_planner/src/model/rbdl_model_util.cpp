		...
		const VectorNd &QDDot, VectorNd &Tau, std::vector<SpatialVector> *f_ext)
{
	...
	for (i = 1; i < model.mBodies.size(); i++) {
		...
		std::cout << i << " " << model.GetBodyName(i) << std::endl << std::endl;
		std::cout << model.f[i] << std::endl << std::endl;
		std::cout << model.mBodies[i].mSpatialInertia << std::endl << std::endl;
		std::cout << model.a[i] << std::endl << std::endl;
		std::cout << model.v[i] << std::endl << std::endl;
		std::cout << model.mBodies[i].mSpatialInertia * model.v[i] << std::endl << std::endl;
		std::cout << crossf(model.v[i],model.mBodies[i].mSpatialInertia * model.v[i]) << std::endl << std::endl;
	...
	for (i = model.mBodies.size() - 1; i > 0; i--) {
		...
		if (lambda != 0) {
			std::cout << lambda <<  " " << i << std::endl << std::endl;
			std::cout << model.f[lambda] << std::endl << std::endl;
			std::cout << model.X_lambda[i] << std::endl << std::endl;
			std::cout << model.f[i] << std::endl << std::endl;
			model.f[lambda] = model.f[lambda] + model.X_lambda[i].applyTranspose(model.f[i]);
			std::cout << model.X_lambda[i].applyTranspose(model.f[i]) << std::endl << std::endl;
			std::cout << model.f[lambda] << std::endl << std::endl;
		}
	}
