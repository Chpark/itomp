...
NewEvalManager::NewEvalManager(int* iteration) :
...
void NewEvalManager::initialize(ItompCIOTrajectory *full_trajectory, ItompCIOTrajectory *group_trajectory,
...
double NewEvalManager::evaluate(Eigen::VectorXd& costs)
{
  double ret = evaluate();
...
double NewEvalManager::evaluate(DERIVATIVE_VARIABLE_TYPE variable_type, int free_point_index, int joint_index)
{
...
void NewEvalManager::setTrajectory(const Eigen::MatrixXd& parameters, const Eigen::MatrixXd& vel_parameters,
...
void NewEvalManager::setTrajectory(const std::vector<Eigen::VectorXd>& parameters,
...
void NewEvalManager::backupAndSetVariables(double new_value, DERIVATIVE_VARIABLE_TYPE variable_type,
...
void NewEvalManager::restoreVariable(DERIVATIVE_VARIABLE_TYPE variable_type, int free_point_index, int joint_index)
{
  Eigen::MatrixXd* target_trajectory;
  switch (variable_type)
  {
...
double NewEvalManager::evaluateDerivatives(double value, DERIVATIVE_VARIABLE_TYPE variable_type,
    int free_point_index, int joint_index)
{
...
void NewEvalManager::render(int trajectory_index)
{
...
void NewEvalManager::computeMassAndGravityForce()
{
  total_mass_ = 0.0;
...
void NewEvalManager::handleTrajectoryConstraint()
{
...
void NewEvalManager::updateFullTrajectory(int point_index, int joint_index)
{
  getFullTrajectory()->copyFromGroupTrajectory(*getGroupTrajectory(), point_index, joint_index);
}

bool NewEvalManager::performForwardKinematics(int begin, int end)
{
...
void NewEvalManager::computeTrajectoryValidity()
{
...
void NewEvalManager::updateCoM(int point)
{
  const KDL::SegmentMap& segmentMap = robot_model_->getKDLTree()->getSegments();
  // compute CoM, p_j
...
#include <iostream>
void NewEvalManager::computeWrenchSum(int begin, int end)
{
...
void NewEvalManager::computeStabilityCosts(int begin, int end)
{
...
void NewEvalManager::computeCollisionCosts(int begin, int end)
{
...
void NewEvalManager::computeFTRs(int begin, int end)
{
  int safe_begin = max(0, begin);
  int safe_end = min(num_points_, end);
...
void NewEvalManager::computeCartesianTrajectoryCosts()
{
...
void NewEvalManager::postprocess_ik()
{
  return;
...
void NewEvalManager::computeBaseFrames(KDL::JntArray& curJointArray, int point)
{
...
int NewEvalManager::getSegmentIndex(int link, bool isLeft) const
{
  string segmentName;
...
void NewEvalManager::getJointIndex(int& groupIndex, int& kdlIndex, int joint, bool isLeft) const
{
  string jointName;
  if (robot_name_.find("hrp4") != std::string::npos)
  {
...
void NewEvalManager::ComputeCollisionFreeLegUsingIK(int legIndex, const KDL::Vector& rootPos,
...
void NewEvalManager::computeSingularityCosts(int begin, int end)
{
